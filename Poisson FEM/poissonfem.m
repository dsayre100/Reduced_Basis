...
% Example usage: w=poissonfem(0,1,1,2,100,100)
function [v,A,b]=poissonfem(xl,xr,yb,yt,M,N,theta)
f=@(x,y) 2*sin(2*pi*y); % define input function data
r=@(x,y) theta;
g1=@(x) 0; % define boundary values on bottom
g2=@(x) 0; % top
g3=@(y) 0; % left side
g4=@(y) 0; % right side
m=M+1; n=N+1; mn=m*n;
